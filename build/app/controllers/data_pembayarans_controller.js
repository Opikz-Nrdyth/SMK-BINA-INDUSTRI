...
import { pembayaranValidator, pembayaranUpdateValidator, tambahPembayaranValidator, } from '#validators/data_pembayaran';
...
export default class DataPembayaranController {
    async index({ request, inertia, session, auth }) {
        ...
        const calculateTotalDibayar = (nominalBayarArray) => {
            return nominalBayarArray.reduce((total, bayar) => {
                const nominal = parseFloat(bayar.nominal || '0');
                return total + (isNaN(nominal) ? 0 : nominal);
            }, 0);
        };
        const data = pembayarans.map((item) => {
            ...
            const riwayatPembayaran = nominalBayarArray
                .filter((bayar) => bayar.tanggal && bayar.nominal)
                .sort((a, b) => {
        ...
        return inertia.render('Pembayaran/Index', {
            pembayaranPaginate: {
                ...
                firstPage: 1,
                nextPage: pembayaranPaginate.currentPage < pembayaranPaginate.lastPage
                    ? pembayaranPaginate.currentPage + 1
                    : null,
                previousPage: pembayaranPaginate.currentPage > 1 ? pembayaranPaginate.currentPage - 1 : null,
            },
            pembayarans: data,
    ...
    async indexSiswa({ inertia, session, auth }) {
        try {
            await auth.check();
            ...
            const totalHutang = data.reduce((total, item) => total + (item.lunas ? 0 : item.sisaPembayaran), 0);
    ...
    calculateTotalDibayar(nominalBayarArray) {
        return nominalBayarArray.reduce((total, bayar) => {
            const nominal = parseFloat(bayar.nominal || '0');
            return total + (isNaN(nominal) ? 0 : nominal);
        }, 0);
    }
    async create({ inertia, session, request }) {
    ...
    async edit({ inertia, session, params }) {
    ...
    async tambahPembayaran({ request, response, session, params }) {
        ...
        try {
            ...
            if (nominalBaru > sisaPembayaran) {
                throw new Error(`Nominal pembayaran (${formatRupiah(String(nominalBaru))}) melebihi sisa pembayaran (${formatRupiah(String(sisaPembayaran))})`);
            }
            if (totalDibayar + nominalBaru > nominalPenetapan) {
                throw new Error(`Total pembayaran akan melebihi nominal penetapan. ` +
    ...
    async partisipasiUjian({ inertia, response, request, session, auth }) {
        try {
            const page = request.input('page', 1);
            const search = request.input('search', '');
            const jenisPembayaran = request.input('jenisPembayaran', '');
