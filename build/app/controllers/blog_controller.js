...
export default class BlogController {
    async index({ request, inertia, session, auth }) {
        ...
        return inertia.render('Blog/Index', {
            blogsPaginate: {
                ...
                firstPage: 1,
                nextPage: blogsPaginate.currentPage < blogsPaginate.lastPage ? blogsPaginate.currentPage + 1 : null,
    ...
    async edit({ inertia, params, auth, session }) {
    ...
    async update({ request, response, session, params, auth }) {
        ...
        try {
            ...
            blog.merge({
                ...payload,
                thumbnail: thumbnailFileName,
                publishedAt: payload.status === 'published' && !blog.publishedAt ? DateTime.now() : blog.publishedAt,
            });
            await blog.save();
            await trx.commit();
            session.flash({
    ...
    async publicShow({ params, inertia }) {
