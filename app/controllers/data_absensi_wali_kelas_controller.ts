// app/controllers/data_absensi_wali_kelas_controller.ts
...
import {
  absensiWaliKelasValidator,
  absensiWaliKelasBulkValidator,
  absensiWaliKelasUpdateValidator,
} from '#validators/data_absensi_wali_kelas'
import type { HttpContext } from '@adonisjs/core/http'
import logger from '@adonisjs/core/services/logger'
import db from '@adonisjs/lucid/services/db'
import { DateTime } from 'luxon'

export default class DataAbsensiWaliKelasController {
  public async index({ request, inertia, session, auth }: HttpContext) {
    ...
    const query = DataAbsensiWaliKelas.query()
    ...
    return inertia.render('AbsensiWaliKelas/Index', {
      absensiPaginate: {
  ...
  public async create({ inertia, auth, session }: HttpContext) {
    ...
    return inertia.render('AbsensiWaliKelas/Create', {
      accessibleKelas,
  ...
  public async store({ request, response, session, auth }: HttpContext) {
    ...
    try {
      const payload = await request.validateUsing(absensiWaliKelasValidator)
      DateTime.fromISO(payload.tanggal)

      const absensiData = payload.absensi.map((item) => ({
        userId: item.userId,
        kelasId: payload.kelasId,
        status: item.status,
        hari: DateTime.fromISO(payload.tanggal),
      ...
      const duplicateChecks = await Promise.all(
        absensiData.map(async (item) => {
          ...
          const existing = await DataAbsensiWaliKelas.query()
      ...
      const uniqueData = absensiData.filter((_, index) => !duplicateChecks[index].exists)
      ...
      if (!isOnline) {
        ...
        // Save ke database
        await DataAbsensiWaliKelas.createMany(uniqueData, { client: trx })
        await trx.commit()
      ...
      await trx.rollback()
      logger.error({ err: error }, 'Gagal menyimpan data absensi wali kelas')
  ...
  public async edit({ inertia, params, session, response, auth }: HttpContext) {
    const absensi = await DataAbsensiWaliKelas.findOrFail(params.id)
    ...
    return inertia.render('AbsensiWaliKelas/Edit', {
      absensi: absensiData,
      users,
      accessibleKelas,
      session: session.flashMessages.all(),
  ...
  public async update({ request, response, session, params, auth }: HttpContext) {
    ...
    try {
      const absensi = await DataAbsensiWaliKelas.findOrFail(id)
      ...
      const payload = await request.validateUsing(absensiWaliKelasUpdateValidator)
  ...
  public async destroy({ response, session, params, auth }: HttpContext) {
    ...
    try {
      ...
      await trx.rollback()
      logger.error({ err: error }, `Gagal hapus data absensi wali kelas`)
      session.flash({
        status: 'error',
  ...
  public async storeBulk({ request, response, session }: HttpContext) {
    ...
    try {
      const payload = await request.validateUsing(absensiWaliKelasBulkValidator)
      ...
      await DataAbsensiWaliKelas.createMany(absensiData, { client: trx })
  ...
  public async getOfflineStats({ response }: HttpContext) {
    try {
      ...
      return response.json({
        success: true,
        message: 'Offline storage API ready untuk absensi wali kelas',
      })
    } catch (error: any) {
      logger.error({ err: error }, 'Error getting offline stats untuk absensi wali kelas')
      return response.status(500).json({
        success: false,
